...
const SystemStatusForm: FC = () => {
	const { loadData, data, errorMessage } = useRest<SystemStatus>({
		read: SystemApi.readSystemStatus,
	});

	const { me } = useContext(AuthenticatedContext);
	const [confirmRestart, setConfirmRestart] = useState<boolean>(false);
	const [confirmFactoryReset, setConfirmFactoryReset] =
	...
	const restart = async () => {
		...
		try {
			...
			enqueueSnackbar(extractErrorMessage(error, "Problem restarting device"), {
				variant: "error",
			});
		} finally {
	...
	const factoryReset = async () => {
		...
		try {
			...
			enqueueSnackbar("Device has been factory reset and will now restart", {
				variant: "info",
			});
		} catch (error: any) {
			enqueueSnackbar(
				extractErrorMessage(error, "Problem factory resetting the device"),
				{ variant: "error" }
			);
		} finally {
				...
				<List>
					...
					<ListItem>
						...
						<ListItemText
							...
							secondary={
								formatNumber(data.sketch_size) +
								" / " +
								formatNumber(data.free_sketch_space) +
								" bytes"
							}
						/>
					</ListItem>
