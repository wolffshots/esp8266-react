...
const WiFiNetworkScanner: FC = () => {
		...
		try {
			...
			if (response.status === 202) {
				...
				if (completedPollCount < NUM_POLLS) {
					...
					finishedWithError(
						"Device did not return network list in timely manner"
					);
				}
			} else {
			...
			finishedWithError(
				extractErrorMessage(error, "Problem listing WiFi networks")
			);
		}
	}, [finishedWithError]);
	...
	const startNetworkScan = useCallback(async () => {
		...
		try {
			...
			finishedWithError(
				extractErrorMessage(error, "Problem scanning for WiFi networks")
			);
		}
	...
	const renderNetworkScanner = () => {
		if (!networkList) {
			return (
				<FormLoader message="Scanning&hellip;" errorMessage={errorMessage} />
			);
		}
